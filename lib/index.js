...
function RpcClient(opts) {
  ...
  this.port = opts.port || 1867;
  this.user = opts.user || 'whive';
...
function innerRpc(request, callback) {
  ...
  const errorMessage = 'Whive JSON-RPC: ';
  ...
  const req = this.protocol.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        const exceededError = new Error(`Whive JSON-RPC: ${buf.toString('utf8')}`);
...
// For definitions of RPC calls, see various files in: https://github.com/whiveio/whive/tree/master/src
RpcClient.callspec = {
